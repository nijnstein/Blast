...
public class Sample12 : MonoBehaviour
{
    ...
    void Start()
    {
        View.text = $"Blast v{Blast.VersionString}\n\nPress [space] to run benchmark once, [B] to run it 100x, [A]rrays, [C]ubes, [E]ditor";
    }
    ...
    unsafe void Update()
    {
        ...
        if (Once || (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.B) || Input.GetKeyDown(KeyCode.A)))
        try{
            Once = false;
            if(Input.GetKeyDown(KeyCode.B) || Input.GetKeyDown(KeyCode.A)) RepeatCount *= 100;
