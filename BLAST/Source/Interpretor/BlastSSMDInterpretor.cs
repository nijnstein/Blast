        ...
        int Execute([NoAlias] byte* syncbuffer, [NoAlias] byte** datastack, int ssmd_data_count, [NoAlias] float4* f4_register)
        {
            ...
            while (code_pointer < package.CodeSize)
            {
                ...
                switch (op)
                {
                    ...
                    case blast_operation.adda:
                        // adda [value] [assignee]
                        DirectlyHandleDataOperation(ref code_pointer, blast_operation.adda);
                        break;
                    ...
                    case blast_operation.suba:
                        // sub from id: a -= 2;
                        DirectlyHandleDataOperation(ref code_pointer, blast_operation.suba);
                        break;

                    case blast_operation.multiply:
                        DirectlyHandleDataOperation(ref code_pointer, blast_operation.multiply);
                        break;

                    case blast_operation.divide:
                        DirectlyHandleDataOperation(ref code_pointer, blast_operation.divide);
                        break;
