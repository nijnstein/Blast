...
#region mula family 
        ...
        void get_all_result(ref int code_pointer, ref BlastVectorSizes vector_size, out float4 f)
        {
            ...
            switch ((BlastVectorSizes)vector_size)
            {
                ...
                case BlastVectorSizes.bool32:
                    {
                        f = math.select(0f, 1f, pop_b32(code_pointer + 1).All); 
                        for(int i = 2; i <= c; i++)
                        {
                            f.x = math.select(0f, 1f, pop_b32(code_pointer + i).All && f.x == 1);
                        }
                        break; 
                    }
                ...
                case BlastVectorSizes.float1:
                    {
                        f = pop_f1(code_pointer + 1) == 0 ? 0f : 1f;
                        //
...
#region Expand Vector 
        ...
        void get_expand_result(ref int code_pointer, ref byte vector_size, in byte expand_to, out float4 f4)
        {
            ...
            {
                case 0:
                case BlastVectorSizes.float4: f4 = new float4(d1[0], d1[0], d1[0], d1[0]); vector_size = 4; break;
...
#endif
                    break;
                case BlastVectorSizes.float2: f4.xy = new float2(d1[0], d1[0]); break;
                case BlastVectorSizes.float3: f4.xyz = new float3(d1[0], d1[0], d1[0]); break;
...
#region Bitwise operations 
        ...
        {
            int dataindex = code[code_pointer + 1] - opt_id;
            ...
            // 
            index = (int)((float*)pop_with_info(code_pointer + 2, out datatype, out vector_size))[0];
            ...
            value = (uint)((float*)pop_with_info(code_pointer + 3, out datatype, out vector_size))[0];
        ...
        void get_setbits_result(ref int code_pointer, ref byte vector_size)
        {
            ...
            // - 1 bit for value, 5 bits for index
            // 
            mask = (uint)((float*)pop_with_info(code_pointer + 2, out datatype, out vector_size))[0];
        ...
        void get_getbits_result(ref int code_pointer, ref byte vector_size, out float4 f4)
        {
            ...
            // - 1 bit for value, 5 bits for index
            // 
            mask = (int)((float*)pop_with_info(code_pointer + 2, out datatype, out vector_size))[0];
        ...
        void get_ror_result(ref int code_pointer, ref byte vector_size)
        {
            ...
            // - 1 bit for value, 5 bits for index
            // 
            amount = (int)((float*)pop_with_info(code_pointer + 2, out datatype, out vector_size))[0];
        ...
        void get_rol_result(ref int code_pointer, ref byte vector_size)
        {
            ...
            if (dataindex < 0 || dataindex + opt_id >= 255)
            {
                Debug.LogError($"rol: first parameter must directly point to a dataindex but its '{dataindex + opt_id}' instead");
                return;
            }
            ...
            if (vector_size != 1)
            {
                Debug.LogError($"rol: to amount to rotate may only be of vectorsize 1 p1 = {datatype}.{vector_size}");
                return;
            }
        ...
        void get_shl_result(ref int code_pointer, ref byte vector_size)
        {
            ...
            if (dataindex < 0 || dataindex + opt_id >= 255)
            {
                Debug.LogError($"shl: first parameter must directly point to a dataindex but its '{dataindex + opt_id}' instead");
                return;
            }
            ...
            if (vector_size != 1)
            {
                Debug.LogError($"shl: to amount to rotate may only be of vectorsize 1 p1 = {datatype}.{vector_size}");
                return;
            }
        ...
        void get_shr_result(ref int code_pointer, ref byte vector_size)
        {
            ...
            if (vector_size != 1)
            {
                Debug.LogError($"shr: to amount to rotate may only be of vectorsize 1 p1 = {datatype}.{vector_size}");
                return;
            }
        ...
        void get_sequence_result(ref int code_pointer, ref byte vector_size, out float4 f4_result)
        {
            ...
            while (code_pointer < package.CodeSize)
            {
                ...
                switch ((blast_operation)op)
                {
                    ...
                    case blast_operation.expand_v4: get_expand_result(ref code_pointer, ref vector_size, 4, out f4); break;

                    // bitwise ops 
                    case blast_operation.set_bit: get_setbit_result(ref code_pointer, ref vector_size); break;
                    case blast_operation.set_bits: get_setbits_result(ref code_pointer, ref vector_size); break;
                    case blast_operation.get_bit: get_getbit_result(ref code_pointer, ref vector_size, out f4); break;
                    ...
                    case blast_operation.assigns:
                        {
                            ...
                            switch ((byte)vector_size)
                            {
                                case (byte)BlastVectorSizes.float1:
                                    ...
                                    {
                                        float x = ((float*)pdata)[0];
                                        switch (s_assignee)
                                        {
                                            case 1: fdata[assignee] = x; break;
                                            case 2: fdata[assignee] = x; fdata[assignee + 1] = x; break;
                                            case 3: fdata[assignee] = x; fdata[assignee + 1] = x; break;
                                            case 0:
                                            case 4: fdata[assignee] = x; fdata[assignee + 1] = x; break;
                                            default:
#if DEVELOPMENT_BUILD || TRACE
                                                Debug.LogError($"blast.assignsingle: assigned vector size mismatch at #{code_pointer}, should be size '{s_assignee}', evaluated '{vector_size}', data id = {assignee}");
#endif
                                                return (int)BlastError.error_assign_vector_size_mismatch;
                                        }
#endif
                    ...
                    case blast_operation.assignf:
                        {
...
#if DEVELOPMENT_BUILD || TRACE || AUTO_EXPAND
                            ...
                            if (s_assignee == vector_size || (is_indexed && vector_size == 1))
                            {
                                // normal set to assignee
                                switch (vector_size)
                                {
                                    case 1:
                                        int offset = math.select(assignee, assignee + (int)(indexer - blast_operation.index_x), is_indexed);
                                ...
                                switch (vector_size)
                                {
                                    case 1:
                                        switch (s_assignee)
                                        {
                                            ...
                                            case 2: fdata[assignee] = f4_register.x; fdata[assignee + 1] = f4_register.x; break;
                                            case 3: fdata[assignee] = f4_register.x; fdata[assignee + 1] = f4_register.x; fdata[assignee + 2] = f4_register.x; break;
                                            case 4: fdata[assignee] = f4_register.x; fdata[assignee + 1] = f4_register.x; fdata[assignee + 2] = f4_register.x; fdata[assignee + 3] = f4_register.x; break;
                                        }
                                        break;
                    ...
                    case blast_operation.assignfn:
                        {
                            ...
                            if (vector_size != 1 && is_indexed)
                            {
                                Debug.LogError($"blast.assignfn: cannot set component from vector at #{code_pointer}, component: {indexer}");
                                return (int)BlastError.error_assign_component_from_vector;
                            }
#endif
                            ...
                            else
                            {
                                ...
                                switch (vector_size)
                                {
                                    ...
                                    default:
                                        Debug.LogError($"blast: assignfn function, vector size mismatch at #{code_pointer}, should be size '{s_assignee}', function returned '{vector_size}', data id = {assignee}");
                                        return (int)BlastError.error_assign_vector_size_mismatch;
                                }
                                vector_size = s_assignee;
                            }
                    ...
                    case blast_operation.assignfe:
                        {
                            ...
                            else
                            {
                                ...
                                switch (vector_size)
                                {
                                    ...
                                    default:
                                        Debug.LogError($"blast: assignfe function, vector size mismatch at #{code_pointer}, should be size '{s_assignee}', function returned '{vector_size}', data id = {assignee}");
                                        return (int)BlastError.error_assign_vector_size_mismatch;
                                }
                                vector_size = s_assignee;
                            }
                    ...
                    case blast_operation.assignfen:
                        {
                            ...
                            else
                            {
                                ...
                                switch (vector_size)
                                {
                                    ...
                                    default:
                                        Debug.LogError($"blast: assignfen function, vector size mismatch at #{code_pointer}, should be size '{s_assignee}', function returned '{vector_size}', data id = {assignee}");
                                        return (int)BlastError.error_assign_vector_size_mismatch;
                                }
                                vector_size = s_assignee;
                            }
